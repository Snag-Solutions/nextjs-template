...
export function GithubMergePRFields({ value, onChange }: RuleSpecificFieldProps) {
  const metadata = value.metadata ?? {}
  const githubRepoUrl = (metadata.githubRepoUrl as string) ?? ''
  const githubBranchName = (metadata.githubBranchName as string) ?? ''
  ...
  return (
    <div className="flex flex-col gap-4">
      <h3 className="text-lg font-semibold">Merge a GitHub PR</h3>
      <p className="text-sm text-gray-600">
        Reward users for merging a pull request to a GitHub repository.
      </p>
      ...
      <label className="flex flex-col gap-1">
        ...
        <input
          ...
          onChange={(e) => updateMetadata({ githubBranchName: e.target.value })}
